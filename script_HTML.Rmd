...
¡Hola! Soy [Ronny Hdez-Mora](https://ronnyhdez.netlify.com/) y he hecho una recopilación de información sobre cómo hacer uso en R de datos que representan fechas y horas. La intención es hacerle un poco más fácil la vida a quién siga este tutorial y necesite una introducción sobre el manejo de este tipo de datos.
...
## ¿Qué va a encontrar en el tutorial?
...
# Análisis estadísticos con datos de fechas. 

Una de las posibles tareas que tengamos, es hacer un análisis estadístico. Digamos que podríamos hacer una regresión o un ANOVA. De acuerdo a la prueba necesitamos datos continuos (regresión) o datos categóricos (ANOVA)

Nosotros podemos ver datos de meses como continuos o como categóricos, pero a R se lo tenemos que hacer explícito y es aquí donde hacemos uso de las clases que vimos anteriormente.

## Fecha continua para regresión

Vamos a hacer una regresión (a modo de ejemplo y no nos vamos a interesar por los supuestos). Vamos a explorar la relación entre la fecha y los retrasos que sufren los vuelos
...
Si la fecha no está como un continuo, es decir con clase **POSIXct** el modelo no se podrá hacer, puesto que para una regresión necesitamos datos continuos.

Si nos encontramos con esto simplemente podemos hacer una coerción de la siguiente manera:

```{r}
data$Rdate <- as.POSIXct(data$Rdate)
```

Y volvemos a repetir el modelo:
...
¿Y en el caso de una ANOVA?

Pues para esto necesitamos datos categóricos y por ende es necesario que tenamos las categorías:
```{r}
